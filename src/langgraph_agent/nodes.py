...
def planner_node(state: AgentState) -> AgentState:
    ...
    state["messages"].append(
        f"[Planner] Created plan with {len(plan)} steps. Routing to: {next_node}"
    )
...
def researcher_node(state: AgentState) -> AgentState:
    ...
    messages = [
        ...
        HumanMessage(
            content=f"Research for this step: {current_step}\n\nContext: {state['input']}"
        ),
    ]
    ...
    state["messages"].append(
        f"[Researcher] Completed research for step {state['current_step'] + 1}"
    )
    ...
    if state["current_step"] >= len(state["plan"]):
        ...
        research_keywords = ["research", "investigate", "find", "explore", "analyze"]
        state["next_node"] = (
            "researcher" if any(kw in next_step.lower() for kw in research_keywords) else "builder"
        )
