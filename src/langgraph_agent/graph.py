...
from langgraph_agent.nodes import planner_node, researcher_node, builder_node
...
def create_agent_graph(max_iterations: int = 3) -> StateGraph:
    ...
    def route_from_builder(state: AgentState) -> Literal["planner", END]:
        ...
        if state.get("iteration", 0) >= max_iterations:
            state["messages"].append(f"[Graph] Max iterations ({max_iterations}) reached. Stopping.")
            state["status"] = "complete"
            return END
        
        # Check if feedback indicates need for replanning
        feedback = state.get("feedback", "")
        if feedback and any(kw in feedback.lower() for kw in ["redo", "fix", "change", "revise", "replan", "wrong", "error"]):
            state["iteration"] = state.get("iteration", 0) + 1
            state["messages"].append(f"[Graph] Feedback detected. Replanning (iteration {state['iteration']}/{max_iterations})")
            return "planner"
